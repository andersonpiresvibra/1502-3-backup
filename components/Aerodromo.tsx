...
import { 
  Search, LayoutGrid, Power, Anchor, Ban, AlertTriangle, Truck, User
} from 'lucide-react';
...
export const Aerodromo: React.FC<AerodromoProps> = ({ onSelectFlight }) => {
              ...
              {currentPositions.filter(p => p.includes(searchTerm)).map(posId => {
                  ...
                  return (
                      <div
                          ...
                          `}
                          onClick={() => !isDisabled && !isReportingCalco && flight && onSelectFlight && onSelectFlight(flight)}
                      >
